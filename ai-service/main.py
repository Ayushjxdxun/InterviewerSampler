    ...
    try:
        if request.interview_type == "coding-mix":
            ...
            instruction = (
                ...
                f"3. {oral_count} questions must be senior-level system design or scenario-based architectural questions."
            )
        else:
            instruction = f"1. Generate exactly {request.count} high-level conceptual/architectural questions. Do not generate basic 'What is X' definitions."

        system_prompt = (
            "You are a Lead Staff Engineer and Technical Interviewer at a top-tier tech company. "
            "YOUR MISSION: Create an extremely challenging interview. "
            "STRICT CONSTRAINTS:\n"
        ...
        user_prompt = (
            ...
            "Generate questions that test deep expertise."
        )
...
async def evaluate(request: EvaluationRequest):
    try:
        assessment_instruction = (
            "This is a conceptual oral question. Focus on deep understanding." if request.question_type == "oral" 
            else "This is a coding challenge. Evaluate logic, efficiency, and edge cases."
        )
        
        system_prompt = (
            "You are a strict, senior-level technical interviewer. "
